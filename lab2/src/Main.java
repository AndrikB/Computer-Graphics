...
public class Main {
  ...
  public static int checkRightOfLine(Point C, ArrayList<Point> line, ArrayList<Point> poligon) {
    ...
    while (wasChange) {
      wasChange = false;
      if (poligon.get(line.get(mid).x).y > C.y) {
        if (r == mid)
          return checkRightOfEdge(poligon.get(line.get(mid).x), poligon.get(line.get(mid).y), C);
        r = mid;
        mid = l + (r - l) / 2;
        wasChange = true;
      } else if (poligon.get(line.get(mid).y).y < C.y) {
        if (l == mid && r != mid) {
          mid = r;
          wasChange = true;
        } else {
      ...
      } else return checkRightOfEdge(poligon.get(line.get(mid).x), poligon.get(line.get(mid).y), C);
    }
  ...
  public static void main(String[] args) throws FileNotFoundException {
    ...
    ArrayList<Point> vetrex = inputData("vetrex.txt");
    ArrayList<Point> edges = inputData("edges.txt");

    ArrayList<ArrayList<Point>> arrayLines = Lines(vetrex, edges);
    ...
    while (true) {
      ...
      Point p = binSearchL(arrayLines, c, vetrex);
  ...
  public static Point binSearchL(
      ArrayList<ArrayList<Point>> lines, Point C, ArrayList<Point> poligon) {
    int n = lines.size();
    int mid = n / 2;
    int r = n - 1;
    int l = 0;

    if (poligon.get(lines.get(0).get(lines.get(0).size() - 1).y).y < C.y) return new Point(-1, 1);
    if (poligon.get(lines.get(0).get(0).x).y > C.y) return new Point(-1, 1);
    if (C.equals(poligon.get(lines.get(0).get(lines.get(0).size() - 1).y)))
      return new Point(mid, 0);
    if (C.y == poligon.get(lines.get(0).get(lines.get(0).size() - 1).y).y) return new Point(-1, -1);

    System.out.println(checkRightOfLine(C, lines.get(l), poligon));
    System.out.println(checkRightOfLine(C, lines.get(r), poligon));

    if (checkRightOfLine(C, lines.get(l), poligon) == -1
        || (checkRightOfLine(C, lines.get(r), poligon) == 1)) return new Point(-1, 1);
